import { getAllUsers, getBerlatih } from "../../util/js/db.js"
...
const populateTable = () => {
  ...
  currentPageData.forEach((data, index) => {
    ...
    if (
      ...
      row.innerHTML = `<td>${startIndex + index + 1}</td><td>${data.name}</td><td>${
        data.nisn
      }</td><td>${data.sekolah}</td><td>${data.kelas}</td><td>${data.nilai}</td><td>${
        data.hari
      }</td><td>${data.waktu}</td><td><button class="p-2" onclick="deleteBerlatih('${
...
const updatePaginationUI = () => {
  const paginationLinks = document.getElementById("pagination").getElementsByTagName("li")
